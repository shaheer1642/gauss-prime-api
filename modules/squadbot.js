        ...
        return new Promise((resolve,reject) => {
            ...
            `).then(res => {
                if (res.rowCount == 1) {
                    ...
                    db_modules.schedule_query(`UPDATE as_sb_squads SET members = members-'${data.discord_id}' WHERE members @> '"${data.discord_id}"' AND status='active' AND squad_id = '${squad_id}'`,squad_expiry)
...
function squadsAddMember(data,callback) {
    ...
    `).then(res => {
        if (res.rowCount == 1) {
            db_modules.schedule_query(`UPDATE as_sb_squads SET members = members-'${data.discord_id}' WHERE members @> '"${data.discord_id}"' AND status='active' AND squad_id = '${data.squad_id}'`,squad_expiry)
            return callback({
                code: 200
            })
        } else return callback({
...
function squadsInvalidate(data,callback) {
    ...
    db.query(`
        UPDATE as_sb_squads SET status = 'invalidated', invalidated_by = '${data.discord_id}', invalidation_reason = '${data.reason}' WHERE status = 'closed' AND squad_id = '${data.squad_id}' AND validated_by is null AND invalidated_by is null;
    `).then(res => {
        if (res.rowCount == 1) {
            return callback({
                code: 200
