...
function squadsInvalidate(data,callback) {
    ...
    db.query(`
        UPDATE as_sb_squads SET status = 'invalidated', invalidated_by = '${data.discord_id}', invalidation_reason = '${data.reason}', invalidated_members = '${JSON.stringify(data.invalidated_members) || '[]'}' WHERE status = 'closed' AND squad_id = '${data.squad_id}' AND validated_by is null AND invalidated_by is null;
    `).then(res => {
        if (res.rowCount == 1) {
            return callback({
                code: 200
