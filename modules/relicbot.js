...
function squadsInvalidate(data,callback) {
    ...
    db.query(`
        UPDATE rb_squads SET status = 'invalidated', invalidated_by = '${data.discord_id}', invalidation_reason = '${data.reason}' WHERE status = 'closed' AND squad_id = '${data.squad_id}';
    `).then(res => {
        if (res.rowCount == 1) {
            return callback({
                code: 200
...
function squadsSelectHost(data,callback) {
    ...
    if (!data.discord_id) return callback({code: 500, err: 'No discord_id provided'})
    db.query(`
        UPDATE rb_squads SET squad_host = '${data.discord_id}' WHERE status = 'opened' AND squad_id = '${data.squad_id}';
    `).then(res => {
        if (res.rowCount == 1) {
            return callback({
                code: 200
...
function statsFetch(data,callback) {
    ...
    `).then(res => {
        ...
        if (data.limit) {
            stats = stats.map((stat,index) => index < data.limit ? stat:null).filter(o => o != null)
        }
        //console.log(stats)
        return callback({
            code: 200,
